 ...
 *
 */
uint32_t parse_variable_length(istream &in) {

    uint32_t value = static_cast<uint32_t>(in.get());


    // 0x80 is 1000 000, we check that X of XYYY YYYY is 1
    // which mean there's still data to read (data are Y)
    if (in.good() && (value & 0x80) ) {
        // 0x7F => 0111 1111, we keep only the 7th first bits
        // (the ones containing actual data)
        value &= 0x7F;

        uint32_t chunk;
        do {
            chunk = static_cast<uint32_t>(in.get());
            // we shift the value by 7 bits, in order to leave
            // place for the next 7 bits of data
            value = (value << 7) + (chunk & 0x7F);
        // if the chunk was also in 1XXX XXXX, we continue to read
        } while (in.good() && (chunk & 0x80) );
    }
 ...
 *
 */
string MidiError::get_error_description() const {
    switch (error) {
    ...
    default:
        return STRING("Unknown MidiError Code (" << error << ").");
  }
}
 ...
 *
 */
string get_midi_event_type_description(MidiEventType type) {
  switch (type) {
 ...
 *
 */
string get_midi_meta_event_type_description(MidiMetaEventType type) {
  switch (type) {
 ...
 *
 */
string const INSTRUMENT_NAMES[INSTRUMENT_COUNT] = {
